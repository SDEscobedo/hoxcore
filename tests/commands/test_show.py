...
def test_show_command_with_type_filter(mock_registry_path, sample_project_content):
    ...
    with patch('hxc.commands.registry.RegistryCommand.get_registry_path', 
               return_value=mock_registry_path):
        with patch('hxc.commands.show.ShowCommand.find_file', 
                  return_value=file_path) as mock_find:
            with patch('hxc.commands.show.ShowCommand.display_file', 
                      return_value=0):
...
def test_show_command_no_registry(mock_registry_path):
    ...
    with patch('hxc.commands.registry.RegistryCommand.get_registry_path', 
               return_value=None):
        with patch('hxc.commands.show.ShowCommand._get_registry_path', 
                  return_value=None):
            with patch('builtins.print') as mock_print:
                result = main(["show", "P-001"])
...
def test_display_file_error_handling(sample_project_content):
    ...
    with patch('builtins.open', side_effect=Exception("Test error")):
        with patch('builtins.print') as mock_print:
            ...
            assert result == 1
            # Check that error message was printed
            assert any("Error reading file" in call[0][0] for call in mock_print.call_args_list)
...
def test_display_pretty_with_all_fields(sample_project_content):
    ...
    full_content = {
        **sample_project_content,
        ...
        "storage": [{"name": "docs", "provider": "gdrive", "url": "https://drive.google.com"}],
        "databases": [{"name": "main_db", "type": "sqlite", "path": "/path/to/db"}],
        "tools": [{"name": "jira", "provider": "atlassian", "url": "https://jira.example.com"}],
        "models": [{"id": "gpt-4", "provider": "openai", "url": "https://api.openai.com"}],
