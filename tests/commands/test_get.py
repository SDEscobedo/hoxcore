...
def temp_registry(tmp_path):
    ...
    create_sample_project(registry_path / "projects", "proj-001", "Test Project 1", "P-001")
    create_sample_project_full(registry_path / "projects", "proj-002", "Test Project 2", "P-002")
    create_sample_program(registry_path / "programs", "prog-001", "Test Program 1", "PG-001")
...
def create_sample_project_full(directory, filename, title, item_id):
    ...
    project_data = {
        ...
        "databases": [
            {"name": "main_db", "type": "sqlite", "path": "/path/to/db"}
        ],
        "tools": [
        ...
        "knowledge_bases": [
            {"id": "kb-001", "url": "https://kb.example.com"}
        ],
...
def test_get_nonexistent_property(mock_get_registry_path, temp_registry):
    ...
    with patch("builtins.print") as mock_print:
        ...
        assert result == 1
        assert any("Unknown property" in call[0][0] for call in mock_print.call_args_list)
...
def test_get_invalid_entity_type(mock_get_registry_path, temp_registry):
    ...
    with patch("hxc.commands.get.EntityType.from_string", side_effect=ValueError("Invalid entity type")):
        with patch("builtins.print") as mock_print:
            result = main(["get", "P-001", "title", "--type", "project"])
...
def test_get_complex_property_invalid_key_filter(mock_get_registry_path, temp_registry):
    ...
    with patch("builtins.print") as mock_print:
        ...
        assert result == 1
        assert any("Invalid key filter format" in call[0][0] for call in mock_print.call_args_list)


@patch("hxc.commands.registry.RegistryCommand.get_registry_path")
def test_get_complex_property_no_match_key_filter(mock_get_registry_path, temp_registry):
...
def test_get_all_scalar_properties(mock_get_registry_path, temp_registry):
    ...
    scalar_properties = ['type', 'uid', 'id', 'title', 'description', 'status', 'start_date']
...
def test_get_all_complex_properties(mock_get_registry_path, temp_registry):
    ...
    complex_properties = ['repositories', 'storage', 'databases', 'tools', 'models', 'knowledge_bases']
...
def test_get_entity_stays_within_registry(mock_get_registry_path, temp_registry):
    ...
    with patch("builtins.print") as mock_print:
        ...
        assert not any("Security error" in str(call) for call in mock_print.call_args_list)
...
@patch("hxc.commands.registry.RegistryCommand.get_registry_path")
def test_get_multiple_entities_same_id_different_types(mock_get_registry_path, temp_registry):
    """Test getting property when multiple entities have same ID in different types"""
    mock_get_registry_path.return_value = str(temp_registry)
    
    # Create a program with same ID as a project
    create_sample_program(temp_registry / "programs", "prog-002", "Test Program 2", "P-001")
