...
def git_registry_with_entities(git_registry):
    ...
    subprocess.run(
        ["git", "add", "."], cwd=git_registry, check=True, capture_output=True
    )
    subprocess.run(
...
class TestDeleteEntity:
    ...
    def test_delete_entity_git_committed_false_when_use_git_false(
        self, registry_with_entities
    ):
...
class TestDeleteEntityAmbiguous:
    ...
    def test_delete_ambiguous_with_type_filter_succeeds(
        self, registry_with_duplicate_uid
    ):
        ...
        assert (
            registry_with_duplicate_uid / "missions" / "miss-duplicate.yml"
        ).exists()
...
class TestDeleteEntityGitIntegration:
    ...
    def test_delete_sequential_commits(self, git_registry_with_entities):
        ...
        with open(
            git_registry_with_entities / "projects" / "proj-proj0002.yml", "w"
        ) as f:
            yaml.dump(project2, f)
