...
from hxc.utils.git import commit_entity_change, find_git_root, git_available
...
class DeleteOperation:
    ...
    def find_entity_files(
        self, identifier: str, entity_type: Optional[EntityType] = None
    ) -> List[Tuple[Path, str]]:
        """
        ...
        for ent_type in entity_types:
            ...
            if not results:
                for file_path in folder_path.glob(f"{prefix}-*.yml"):
                    try:
                        secure_file_path = resolve_safe_path(
    ...
    def delete_with_git(
        self, file_path: Path, entity_data: Dict[str, Any], entity_type: str
    ) -> bool:
        """
        ...
        rm_result = subprocess.run(
            ["git", "rm", rel_path], cwd=git_root, capture_output=True, text=True
        )
    ...
    def delete_entity(
        ...
        *,
        ...
        if use_git:
            git_committed = self.delete_with_git(
                file_path, entity_data, entity_type_str
            )
        else:
            self.delete_file(file_path)
