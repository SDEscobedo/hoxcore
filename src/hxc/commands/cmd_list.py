...
class ListCommand(BaseCommand):
    ...
    def register_subparser(cls, subparsers) -> argparse.ArgumentParser:
        ...
        parser.add_argument("--parent", help="Filter by parent ID")
        parser.add_argument("--id", help="Filter by ID or UID")
        parser.add_argument("--query", "-q", help="Search in title and description")
        parser.add_argument("--before", help="Filter by due date before YYYY-MM-DD")
        parser.add_argument("--after", help="Filter by due date after YYYY-MM-DD")
        ...
        try:
            ...
            try:
                ...
                status_filter = (
                    None
                    if args.status == "any"
                    else EntityStatus.from_string(args.status)
                )
            ...
            if not registry_path:
                print(
                    "❌ No registry found. Please initialize or set a registry first."
                )
                return 1
    ...
    @classmethod
    def _display_items(
        cls, items: List[Dict[str, Any]], output_format: OutputFormat
    ) -> None:
    ...
    def _display_table(cls, items: List[Dict[str, Any]]) -> None:
        ...
        widths = {
            ...
            "id": max(
                2, max(len(str(item.get("id", item.get("uid", "")))) for item in items)
            ),
        ...
        for item in items:
            ...
            print(
                f"{item_type:<{widths['type']}} {item_id:<{widths['id']}} {item_title:<{widths['title']}} {item_status:<{widths['status']}}"
            )
    ...
    def _display_yaml(cls, items: List[Dict[str, Any]]) -> None:
        ...
        clean_items = ListOperation.clean_entities_for_output(
            items, remove_file_metadata=True
        )
