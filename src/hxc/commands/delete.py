...
class DeleteCommand(BaseCommand):
    ...
    def register_subparser(cls, subparsers):
        ...
        parser.add_argument(
            "--type",
            "-t",
            choices=list(cls.ENTITY_FOLDERS.keys()),
        ...
        parser.add_argument(
            "--no-commit", action="store_true", help="Do not commit deletion to git"
        )
    ...
    def execute(cls, args):
        try:
            ...
            if not args.force:
                print(
                    f"⚠️  Warning: About to delete {entity_type_str} '{entity_title}' at {file_path}"
                )
    ...
    def _find_entity_files(
        cls, registry_path: str, identifier: str, entity_type: Optional[str] = None
    ) -> List[Tuple[str, str]]:
        """
