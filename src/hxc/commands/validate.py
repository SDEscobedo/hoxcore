...
class ValidateCommand(BaseCommand):
    ...
    def _load_all_entities(
        cls, registry_path: str, verbose: bool
            ...
            for file_path in type_dir.glob("*.yml"):
                try:
                    ...
                    if verbose:
                        ...
                        print(
                            f"  ✓ Loaded {entity_type.value}: {uid} ({file_path.name})"
                        )
    ...
    def _validate_required_fields(
        cls, entities: List[Dict[str, Any]], verbose: bool
    ) -> List[str]:
    ...
    @classmethod
    def _validate_uids(cls, entities: List[Dict[str, Any]], verbose: bool) -> List[str]:
    ...
    @classmethod
    def _validate_ids(cls, entities: List[Dict[str, Any]], verbose: bool) -> List[str]:
        ...
        if verbose and not errors:
            print(
                f"  ✓ All {total_ids_checked} IDs are unique within their entity types"
            )
